<template>
  <div class="contents_editor_wrapper curation" v-bind:class="c_screen_mode_class_name">
    <div class="curation_contents_wrapper" ref="curation_contents_wrapper">
      <div class="curation_contents_mid" ref="content_mid">
        <div class="center_flex_container">
          <div class="curation_player_wrapper">
...
<!--            <template v-else-if="c_is_data_load_complete">-->
              ...
              <template><!--  v-if="c_is_curation_editable" -->
              ...
              <div class="divider">
                ...
                <div class="data_info">
                  ...
                  <div class="data_info_others">
                    ...
                    <div class="btn f13 w130 h30 ml_20" v-on:click="onOperationInfoClick"><span>{{ '수술정보 확인/수정' }}</span></div>
                    <template v-if="c_is_drive">
                      <div class="btn f13 w80 h30" v-on:click="onLoadQuestionnaireListPopup"><span>{{ '조사지 등록' }}</span></div>
                    </template>
                    <template v-if="c_video_download">
                      <div class="btn blue w80 f13 h30" v-on:click="onDownloadClick"><span>{{ '다운로드' }}</span></div>
                    </template>
...
export default {
  ...
  components: { VideoPlayer, VLazyImage, AnnotationList, AnnotationToolBar, AnnotationDraw },
  mixins: [BaseMixin],
  data() {
    return {
  ...
  computed: {
    ...mapGetters(['lang', 'group_seq', 'group_grade_number', 'member_seq', 'is_admin', 'is_group_admin', 'is_personal_group', 'use_nickname', 'folder_info', 'lang']),
    ...
    c_is_drive() {
      return this.api_type === 'drive';
    },
    c_is_admin() {
      return this.api_type === 'admin';
    },
  ...
  methods: {
    ...
    loadFollowState() {
      api_follow.getFollowState(this.writer_info.group_seq)
        .then((result) => {
          if (result && result.follow_info) {
            this.follow_info = result.follow_info;
          } else {
    ...
    addAnnotationUser(clip_info, is_delete = false) {
      ...
      if (!this.annotation_user_map[member_seq]) {
        ...
        this.$set(this.annotation_user_map, member_seq, {
          ...
          name: this.use_nickname ? clip_info.user_nickname : clip_info.user_name,
          display_name: this.use_nickname ? clip_info.user_nickname : clip_info.user_name,
          modify_date,
          count: 0,
          is_me,
          order: is_me ? 1 : 101,
        ...
        this.annotation_user_map[member_seq].name = this.use_nickname ? clip_info.user_nickname : clip_info.user_name;
      }
      this.annotation_user_map[member_seq].count++;
      this.annotation_user_map.all.count++;
      this.clip_count++;
