  ...
  export default {
    ...
    methods: {
      ...
      draw(shape_data) {
        ...
        const width = Math.abs(this.shape_data.width) - padding;
        const height = Math.abs(this.shape_data.height) - padding;

        if (width > 0 && height > 0) {
          this.drawRect(shape_data, false, this.hit_ctx, padding);
          this.drawRect(shape_data, false, this.ctx, padding);
          this.is_empty = false;
        } else {
          this.is_empty = true;
        }
