...
<!--              <div class="btn h35 pad10 red_line_text" v-on:click="onRemoveItemsClick(true)"><img src="/img/renewal/drive/red_delete_icon.png"/><span>휴지통 비우기</span></div>-->
            ...
            <template>
              <template>
                <div class="btn blue h35 pad10" v-on:click="onRegClick"><span>사용승인</span></div>
              </template>
              <template>
                <div class="btn white h35 pad10" v-on:click="onRegClick"><span>사용정지</span></div>
              </template>
              <template>
                <div class="btn red h35 pad10" v-on:click="onRegClick"><span>삭제</span></div>
              </template>
              <template>
                <div class="btn blue h35 pad10" v-on:click="onRegClick"><span>등록</span></div>
              </template>
            </template>
          </div>
        ...
        <template>
          <div class="content">
            ...
            <div :class="c_folder_class">
              <template v-for="(operation_info, index) in c_operation_infos">
                <OperationListItem
...
import ModuleOperationCreate from './ModuleMemberJoin';
import Pages3 from '../../../pages/OperationView';

export default {
  name: 'MemberList',
  components: { OperationListHeader, OperationListItem },
  props: ['type'],
  ...
  methods: {
    ...
    videoplay(operation_info) {
      ...
      params.is_List = '3';
      this.$router.push({ name: 'page4', params });
    },
    addEventListener() {
...
.effect_bottom_border {
  border-bottom: 1px solid transparent;
}
.effect_bottom_border:hover {
  border-bottom: 1px solid #555555;
}
.empty_min_height.tutorial {
  flex: 0;
  flex-basis: 300px;
  min-height: 300px;
